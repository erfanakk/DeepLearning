...
class Discriminator(nn.Module):
    ...
    def __init__(self , in_channels , n_feature, img_size, num_classes):
        super().__init__()
        #input 3*64*64
        self.img_size = img_size
        self.dic = nn.Sequential(
            nn.Conv2d(in_channels+1, n_feature, kernel_size=4, stride=2, padding=1), #32*32
            nn.LeakyReLU(0.2),
...
class Generator(nn.Module):
    def __init__(self, z_dim, n_feature, img_chan, num_classes, img_size, embed_size):
        ...
        self.gen = nn.Sequential(
            self._block(z_dim+embed_size, n_feature*16, kernel_size=4, stride=1, padding=0),# N,(n_feature*16)*4*4
